...
class BezierCurve:
    ...
    def fit0(self, tpara=[]):
        ...
        if BezierCurve.openmode:
            exA = np.array([[sum([bs(i, t[k])*bs(n, t[k]) for k in range(M)])
                             for i in range(N+1)] for n in range(N+1)], 'float64')
            while np.linalg.matrix_rank(exA) < exA.shape[0]:
                t[1] = t[1]+0.000001
            exBX = np.array([[sum([x[k]*bs(n, t[k]) for k in range(M)])]
                             for n in range(N+1)], 'float64')
            exBY = np.array([[sum([y[k]*bs(n, t[k]) for k in range(M)])]
                             for n in range(N+1)], 'float64')
            ...
            while np.linalg.matrix_rank(exA) < exA.shape[0]:
                t[1] = t[1]+0.000001
            exBX = np.array([[sum([bs(n, t[k])*(x[k]-x[0]*(1-t[k])**N - x[-1]*t[k]**N)
                                   for k in range(M)])] for n in range(1, N)], 'float64')
            exBY = np.array([[sum([bs(n, t[k])*(y[k]-y[0]*(1-t[k])**N - y[-1]*t[k]**N)
                                   for k in range(M)])] for n in range(1, N)], 'float64')
    ...
    # 段階的ベジエ近似
    def fit2(self, mode=0, Nprolog=3, Nfrom=5, Nto=12, preTry=1000, maxTry=0, lr=0.03, prefunc=None, pat=10, err_th=0.75, threstune=1.0, withErr=False, tpara=[], withFig=False):
    ...
    def setParameters(priority='distance', dCount=7, debugmode=False, openmode=False):
